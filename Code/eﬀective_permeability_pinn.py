...
def train_model(model, training_points, weights, optimizer, scheduler, epochs):
    """
    训练PINN模型
    Args:
        model: PINN模型
...
def lbfgs_optimization(model, training_points, weights):
    """
    使用L-BFGS优化器进行微调
    Args:
        model: PINN模型
...
def train_model_with_checkpoints(model, training_points, weights, optimizer, scheduler, 
                              epochs, rock_type, checkpoint_interval=1000):
    """
    带检查点的模型训练
    Args:
        model: PINN模型
...
def main():
    ...
    for rock_type in rock_types:
        ...
        if args.train:
            ...
            # L-BFGS微调
            lbfgs_optimization(model, training_points, weights)
    ...
    if len(rock_types) == 2:
        ...
        if stanstead_k_geometric and lac_du_bonnet_k_geometric:
            # 保存为PINN结果字典
